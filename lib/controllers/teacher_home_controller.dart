...
class TeacherHomeController extends GetxController {
  ...
  Future<void> _loadLocalData() async {
    ...
    if (localEmplois != null) {
      ...
      final transformed = localEmplois.map((s) {
        ...
        final emploi = map['emploiDuTemps'] as Map? ??
            map; // Fallback pour l'ancien format
        ...
        if (seance != null && seance['statut'] != null) {
          ...
          final calculated = _calculateStatus(date,
              emploi['heureDebut']?.toString(), emploi['heureFin']?.toString());
    ...
    if (cachedAbsences != null && cachedAbsences['absences'] != null) {
      absencesCount.value =
          int.tryParse(cachedAbsences['absences'].toString()) ?? 0;
    }
  ...
  Future<void> fetchDashboardData() async {
    ...
    try {
      ...
      final date = DateTime.now();
      final fetchedEmploisAvecSeances =
          await _repo.getEmploisDuTempsParDate(date);
      if (fetchedEmploisAvecSeances is List) {
        ...
        final transformedSeances = fetchedEmploisAvecSeances.map((item) {
          ...
          if (seance != null && seance['statut'] != null) {
            ...
            final calculated = _calculateStatus(
                date,
                emploi['heureDebut']?.toString(),
                emploi['heureFin']?.toString());
            if (calculated == 'EN_COURS') {
              statut = 'En cours';
            } else if (calculated == 'TERMINEE') {
              statut = 'Terminé';
      ...
      if (absencesData != null && absencesData['absences'] != null) {
        absencesCount.value =
            int.tryParse(absencesData['absences'].toString()) ?? 0;
      } else {
  ...
  Future<void> demarrerSeance(int index) async {
    ...
    if (affectationId == null) {
      Get.snackbar('Erreur',
          'Impossible de démarrer la séance : affectation introuvable',
  ...
  Future<void> terminerSeance(int index) async {
    ...
    if (seanceId == null) {
      Get.snackbar(
          'Erreur', 'Impossible de terminer la séance : séance introuvable',
          snackPosition: SnackPosition.BOTTOM);
      return;
    }
    try {
